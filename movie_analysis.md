...
In this dataset, highest number of movies are produced in ```U.S.A```, close to 3500. In the bivariate anlaysis, we will look at different characteristics, including ```language```,```content_rating```, etc. in relation to countries. Lets look at the top 10 countries with respect to number of movies.
...
The distribution is right skewed, as expected. Most of the movies have a low budget. Out of curiosity, we identify the movie with highest budget.
...
Similar to ```genre```, does movie count for different ```content_rating``` also vary wih time?
